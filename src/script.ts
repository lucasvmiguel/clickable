...
windowObj.addEventListener('load', () => {
  ...
  Promise.resolve(response).then((response) => { apiResponse = response });

  const likeIconMarkup = `
    ...
    return `
      <div id="clickable-menu" style="display: block">
        <ul class="clickable-menu-options">
          ${items.map(i => itemMarkup(i)).join('')}
        </ul>
      </div>
    `;
  };

  const getPositionClicked = event => ({
    left: event.pageX,
    top: event.pageY
  });
  ...
  const getFirstClickableAttribute = node => {
    if (!node || !node.getAttributeNode) {
      return null;
    }

    const valueAttr = node.getAttributeNode("data-clickable-value");
    const menuAttr = node.getAttributeNode("data-clickable-menu");
  ...
  const createMenu = ({ apiResponse, menuType }) => {
  ...
  windowObj.addEventListener("click", e => {
    ...
    if (menuVisible && id) {
      sendClickItem({ id, value: menuValue });
    };
  ...
  windowObj.addEventListener("contextmenu", e => {
    ...
    const menuType = attrs && attrs.menu;

    createMenu({ apiResponse, menuType });
